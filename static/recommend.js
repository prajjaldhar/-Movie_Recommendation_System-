$(function() {
    ...
    $('.movie-button').on('click',function(){
      var my_api_key = 'ecc9f726de956f2008a471db9b8834fa';
      var title = $('.movie').val();
      if (title=="") {
  ...
  function movie_recs(title,movie_id,my_api_key){
    $.ajax({
      type:'POST',
      url:"/similarity",
      data:{'name':title},
      success: function(recs){
        if(recs=="Sorry! The movie you requested is not in our database. Please check the spelling or try with some other movies"){
          ...
          get_movie_details(movie_id,my_api_key,arr,title);
        }
      },
      error: function(){
        alert("error recs");
  ...
  function get_movie_details(movie_id,my_api_key,arr,title) {
    $.ajax({
      type:'GET',
      url:'https://api.themoviedb.org/3/movie/'+movie_id+'?api_key='+my_api_key,
      success: function(movie_details){
        show_details(movie_details,arr,title,my_api_key,movie_id);
      },
      error: function(){
        alert("API Error!");
        $("#loader").delay(500).fadeOut();
  ...
  function show_details(movie_details,arr,title,my_api_key,movie_id){
